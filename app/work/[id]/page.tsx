...
import { getWorkItem, EVENT_LABEL, nextEvents } from "@/lib/ledger";
import { requireRole } from "@/lib/auth";
import { SubmitDraftForm } from "@/components/work/submit-draft";
...
export default async function WorkerWorkItemPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  // Workers can only see items assigned to them
  if (user.role === "worker" && item.assigneeId !== user.id) {
    redirect("/work");
  }

  const canSubmitDraft = nextEvents(item.currentStatus).includes("draft_submitted");
      ...
      <Container className="py-10">
        ...
        <div className="mt-4 flex flex-wrap items-start justify-between gap-4">
          <div>
            ...
            <p className="mt-1 text-sm text-muted">
              {item.clientName} · {item.type} · {item.guild}
            </p>
          </div>
        </div>
        ...
        <div className="mt-8 grid gap-8 lg:grid-cols-[1fr_1.2fr]">
          <div className="grid gap-6">
            ...
            {canSubmitDraft && <SubmitDraftForm workItemId={item.id} />}
          </div>
